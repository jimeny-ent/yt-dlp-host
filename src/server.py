...
def get_file(filename):
    if USE_GCS:
        ...
        file_blob = storage_manager.get_file(filename)
        if not file_blob:
            return jsonify({"error": "File not found"}), 404

        # Special handling for info.json files
                    ...
                    for key, value in params.items():
                        if key in data:
                            ...
                            for f in data['formats']:
                                if f.get('format_note') in ['unknown', 'storyboard']: 
                                    continue
        ...
        # For regular files, stream from GCS
        return send_file(
            file_blob.open('rb'),
            mimetype=file_blob.content_type,
            as_attachment=True,
